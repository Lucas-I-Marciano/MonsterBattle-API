  ...
  .post("/", async (req, res) => {
    ...
    if (haveNullValue.length > 0) {
      return res
        ...
        .json({
          message: "fail",
          detail:
            "All fields are required - name, hp, attack, defense, speed, special",
        });
    }
    ...
    try {
      ...
      if (monster) {
        return res
          .status(400)
          .json({ message: "fail", detail: "Monster already exists" });
      }
  ...
  .get("/:id", async (req, res) => {
    ...
    try {
      ...
      if (!monster) {
        return res
          .status(404)
          .json({ message: "fail", detail: "Monster not founded" });
      }
