    ...
    <div className={cardClass}>
      ...
      <div className="p-4 border-t mx-8 mt-2">
        <div className="flex gap-5 pb-5">
          ...
          <button disabled={buttonState.disabled} onClick={() => {
            setCardClass(`${cardClass} ${actions("duration-1000 rotate-270").modifiers}`)
            dispatch({ type: "loading" });
          }} className={`w-1/2 block mx-auto rounded-full ${buttonState.bg} hover:shadow-lg font-semibold text-white px-6 py-2`}>
            Defend
          </button>
        ...
        <div className="flex gap-5">
          <button disabled={buttonState.disabled} onClick={() => {
            setCardClass(`${cardClass} ${actions("duration-1000 scale-120 ").modifiers}`)
            dispatch({ type: "loading" });
          ...
          <button disabled={buttonState.disabled} onClick={() => {
            setCardClass(`${cardClass} ${actions("duration-1000 scale-25 ").modifiers}`)
            dispatch({ type: "loading" });
          }} className={`w-1/2 block mx-auto rounded-full ${buttonState.bg} hover:shadow-lg font-semibold text-white px-6 py-2`}>
            Forfeit
          </button>
